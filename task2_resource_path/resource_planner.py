...
def plan_optimal_resource_path(maze: MazeGame) -> ResourcePlan:
    ...
    def try_trigger_resource_at(current_bitmap: int, pos: Position) -> int:
        if pos in resource_index:
            return current_bitmap | (1 << resource_index[pos])
...
def plan_global_optimal_collection(maze: MazeGame) -> ResourcePlan:
    ...
    for i, start_pos in enumerate(resource_cells):
        dist[i][i] = 0
        ...
        while queue:
            ...
            if curr != start_pos and curr in resource_set:
                continue
        ...
        for j, end_pos in enumerate(resource_cells):
    ...
    resource_cells_in_order = [resource_cells[u] for u in resource_sequence]
    ...
    for index, pos in enumerate(walk_path):
        gain_here = 0
        if pos in resource_set and pos not in triggered:
            triggered.add(pos)
            gain_here = _cell_first_gain(maze, pos)
            running_resource += gain_here
